...
export default function CartDrawer({ isOpen, onClose }: Props) {
  const { items, dispatch, totalItems, subtotalINR, subtotalUSD,
    discountINR, discountUSD, totalINR, totalUSD, coupon, discountPct } = useCart()
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-cosmic-black/70 backdrop-blur-sm z-[90]"
          onClick={onClose}
        />
      )}
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-cosmic-gold/15">
          <div className="flex items-center gap-3">
            ...
            <span className="font-cinzel text-cosmic-cream font-semibold tracking-wide">
              Your Cart
            </span>
            {totalItems > 0 && (
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-16">
              ...
              <p className="font-cormorant text-cosmic-cream/30 text-base italic mt-1">
                Add some healing crystals to begin
              </p>
              <button
                onClick={onClose}
            ...
            items.map(item => (
              <div key={item.id} className="flex gap-4 pb-4 border-b border-cosmic-gold/10">
                {/* Image */}
                <div className="relative w-16 h-16 rounded-sm overflow-hidden shrink-0 border border-cosmic-gold/20">
                  {item.image && !item.image.includes('placeholder') ? (
                    <Image src={item.image} alt={item.name} fill className="object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center bg-cosmic-black/30">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-cinzel text-cosmic-cream text-xs font-semibold line-clamp-2 mb-1 leading-snug">
                    {item.name}
                  </p>
                ...
                <div className="flex flex-col items-end gap-2">
                  <button
                    ...
                    className="text-cosmic-cream/30 hover:text-red-400 transition-colors"
                  >
                    <Trash2 size={13} />
                  </button>
                  <div className="flex items-center gap-1 border border-cosmic-gold/20">
                    <button
                      ...
                      className="w-6 h-6 flex items-center justify-center text-cosmic-cream/60 hover:text-cosmic-gold transition-colors"
                    >
                      <Minus size={10} />
                    </button>
                    <span className="font-raleway text-xs text-cosmic-cream w-5 text-center">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => dispatch({ type: 'UPDATE_QTY', id: item.id, qty: item.quantity + 1 })}
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-cosmic-gold/15 space-y-4">
            ...
            {couponStatus !== 'idle' && couponMsg && (
              <p className={`font-raleway text-xs ${couponStatus === 'ok' ? 'text-green-400' : 'text-red-400'}`}>
                {couponMsg}
              </p>
            )}
            ...
            <div className="space-y-1.5 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-cinzel text-cosmic-cream text-xs">
                  {symbol}{isIndia ? subtotal.toLocaleString('en-IN') : (subtotal as number).toFixed(2)}
                </span>
              </div>
              {discount > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="font-cinzel text-green-400 text-xs">
                    -{symbol}{isIndia ? discount.toLocaleString('en-IN') : (discount as number).toFixed(2)}
                  </span>
                </div>
              )}
              {isIndia && (
                <div className="flex justify-between">
                  ...
                  <span className="font-raleway text-cosmic-cream/40 text-xs">
                    {totalINR >= 1999 ? 'FREE' : '₹99'}
                  </span>
                </div>
              )}
              <div className="flex justify-between pt-2 border-t border-cosmic-gold/15">
                ...
                <span className="font-cinzel text-cosmic-gold font-bold">
                  {symbol}{isIndia ? total.toLocaleString('en-IN') : (total as number).toFixed(2)}
                </span>
              </div>
            </div>
